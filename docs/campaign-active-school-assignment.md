# Incorporación de escuelas durante una campaña activa

La respuesta funcional final habilita al administrador central a incorporar
una escuela nueva cuando una campaña ya está activa. A partir de la asignación,
la escuela aparece en su listado de campañas y puede comenzar el diagnóstico
...
- `POST /api/admin/campaigns/:id/schools/preview` y
  `POST /api/admin/campaigns/:id/schools/assign` aceptan campañas `draft` y
  `active`.
...
- Una campaña `active` cuya fecha de fin ya venció no acepta incorporaciones,
  aunque el cierre periódico todavía no haya actualizado su estado.
- Las campañas `closed` y `archived` no aceptan incorporaciones.
- `DELETE /api/admin/campaigns/:id/schools/:schoolId` continúa habilitado sólo
  en `draft`. El alta durante una campaña activa no permite retirar escuelas de
  su universo histórico.
- Repetir una asignación vigente no crea otra fila. Una asignación previamente
  removida puede reactivarse, conservando el mismo par único campaña/escuela.
...
estado de la campaña, cantidades, identificadores efectivamente asignados y
los identificadores reactivados.

La operación bloquea la fila de la campaña dentro de la misma transacción que
persiste las asignaciones. Los cambios de estado utilizan el mismo bloqueo. En
una campaña activa también bloquea las escuelas nuevas para serializar el alta
